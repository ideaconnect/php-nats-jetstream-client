...
- `[bugfix]` Restore inbound hot-path throughput lost in #140 and drop the extra per-chunk read
  ...
  `explode(' ')` + per-token canonical scan fast path that measured ~27% slower per line than the
  `preg_split('/\s+/')` it replaced on a PCRE-JIT build (JIT is on by default since PHP 7.3 and this
  package requires 8.2+): the canonical scan cost more than the JIT-compiled regex on typical
  single-space lines, and any non-canonical line then ran `preg_split()` anyway. The split reverts
