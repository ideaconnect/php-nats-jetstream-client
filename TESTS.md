...
- `testSubscribeOrderedConsumerRecreatesOnMissedHeartbeats` - the idle-heartbeat watchdog recreates an ordered consumer whose transport goes totally silent (no data, heartbeat, or flow-control) for more than two intervals: a second CONSUMER.CREATE (from opt_start_seq) reaches the wire with no error, detecting a reaped/lost consumer the sequence-gap logic never sees (#113).
...
- `testWatchSilentConsumerSurfacesNotActiveError` - a KV watch requests an idle heartbeat (tunable via KeyWatchOptions::$idleHeartbeat) so its missed-heartbeat watchdog surfaces exactly one "not active" JetStreamException (naming stream KV_cfg) when the watch consumer goes silent, and never deletes/recreates it; previously the default KV watch requested no heartbeat and hung forever (#113).
...
- `testFrameRequiredBytesRejectsOversizedDeclaredLength` - Asserts frameRequiredBytes() throws ProtocolException matching "payload length out of bounds" for a declared length of MAX_FRAME_PAYLOAD+1, same as decode() (#164).
...
- `testParseFrameHeaderRejectsOversizedDeclaredLength` - Asserts parseFrameHeader() throws ProtocolException matching "payload length out of bounds" the moment the hostile length bytes arrive, even with the mask key still in flight (#164).
- `testUnmaskInvertsEncodeMasking` - Asserts unmask() with the frame's key restores the original payload from the masked payload bytes of an encoded client frame (#164).
